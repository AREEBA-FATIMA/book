# Architectural Plan: AI/Spec-Driven Book Creation – Physical AI & Humanoid Robotics Textbook (Phase 2)
...
### Architecture Validation: Backend Repository Structure
- **Decision**: The backend will be a folder (`/backend`) within the current monorepo.
- **Options Considered**: Monorepo (`/backend` folder), Separate repository.
- **Trade-offs**:
    - *Monorepo*: Simpler dependency management, easier local development, unified CI/CD. Can lead to a larger repository size.
    - *Separate repo*: Clear separation of concerns, independent versioning/deployment. Can add complexity for cross-repo dependencies and local development setup.
- **Rationale**: A monorepo structure with the backend in a `/backend` folder is recommended for simplicity, aligning with the project's current setup and easing local development and CI/CD processes.
...
- Potential ADRs:
    - ADR for Authentication Strategy (JWT vs. Session) - *Addressed via plan decisions.*
    - ADR for Production Database Choice (PostgreSQL vs. other SQL/NoSQL) - *Addressed via plan decisions.*
    - ADR for Backend Hosting Provider (Render vs. other cloud providers) - *Addressed via plan decisions.*
    - ADR-002: Use Monorepo for Backend - *Suggested by user prompt, will be created if user approves.*

## Phased Implementation Strategy

### Phase 2.1: Backend Foundation
- Setup FastAPI project structure in the `backend/` directory.
- Configure Qdrant client connection and API keys via environment variables.
- Define and load environment variables for database connections, LLM API keys, and other secrets.

### Phase 2.2: RAG Pipeline Implementation
- Implement the ingestion script (`scripts/ingest.py`) to scrape Docusaurus content, chunk it, generate embeddings, and upload to Qdrant.
- Execute an initial run of the ingestion script to populate Qdrant with book content.
- Develop the `POST /api/chat` endpoint in FastAPI, integrating Qdrant for retrieval and the LLM provider for generation.

### Phase 2.3: Authentication System
- Install and configure Better-Auth within the FastAPI application.
- Set up the User Database schema (SQLite for development) and integrate with FastAPI.
- Implement `POST /api/auth/*` (login, signup, logout) endpoints and `GET /api/user/profile`, `POST /api/user/progress` endpoints.

### Phase 2.4: Frontend Integration
- Create a new React `ChatWidget` component in Docusaurus, capable of interacting with the `/api/chat` endpoint.
- Integrate Login/Logout buttons and user profile links into the Docusaurus `NavBarAuth` component, connecting to the authentication API.
- Implement the API client (Axios/Fetch) in the Docusaurus frontend to communicate with the FastAPI backend.

## Verification Strategy
- **RAG Accuracy Testing**: Develop unit tests for the ingestion script, integration tests for retrieval logic (Qdrant queries), and end-to-end tests for chat interactions, evaluating response relevance and citation accuracy against expected answers.
- **Authentication Security Verification**: Conduct security audits, implement automated tests for common vulnerabilities (e.g., injection, unauthorized access), and perform penetration testing (if applicable) to ensure secure token management and user data protection.

## Risk Assessment & ADRs
- **Potential Blockers**:
    - **CORS Issues**: Cross-Origin Resource Sharing (CORS) configuration between Docusaurus (frontend) and FastAPI (backend) could be a blocker. Mitigation: Implement robust CORS handling in FastAPI.
    - **Build Process Complexity**: Integrating a Python backend into a JavaScript/TypeScript frontend monorepo might add build complexity. Mitigation: Use separate build commands/scripts for frontend and backend, orchestrated by CI/CD.
    - **LLM API Rate Limits/Cost**: Exceeding LLM API rate limits or incurring unexpected costs during development/testing. Mitigation: Implement caching, monitor usage, use smaller models for development.
- **Suggested Architectural Decision Records (ADRs)**:
    - ADR-002: Monorepo Backend Structure (Rationale: Simplicity for local development and unified CI/CD)

**Plan Version**: 1.0.0 | **Created**: 2025-12-06 | **Last Modified**: 2025-12-06
